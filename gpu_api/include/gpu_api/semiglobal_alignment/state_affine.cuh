#ifndef SEMIGLOBAL_ALIGNMENT_STATE_AFFINE_CUH
#define SEMIGLOBAL_ALIGNMENT_STATE_AFFINE_CUH
...
namespace semiglobalalignment{
    ...
    // #define PRINT_STATE_SEMIGLOBAL_AFFINE
    // #define PRINT_STATE_SEMIGLOBAL_AFFINE_HALF2

    #define USE_FIVE_MAX_FOUR_ADD_VERSION
    ...
    >
    struct SemiglobalAlignmentAffineGapState_floatOrInt{
        ...
        __device__
        SemiglobalAlignmentAffineGapState_floatOrInt(const SubstitutionScoreProvider& sub, Group& g, UpdateMaxInLastColumnOp& update, const ScoringKernelParam<ScoreType>& scoring
        ) : gapopenscore(scoring.gapopenscore), gapextendscore(scoring.gapextendscore), group(g), substitutionScores(sub), updateMaxInLastCol(update){}
        ...
        #ifdef USE_FIVE_MAX_FOUR_ADD_VERSION
        ...
        void relaxFirstDiagonal_impl(int currentLetter, int row, int tileNr){
            ...
            if constexpr(isFirstTile){
                ...
                #ifdef PRINT_STATE_SEMIGLOBAL_AFFINE
            ...
            for(int i = 1; i < numItems/relaxChunkSize; i++){
                ...
                for(int k = 0; k < relaxChunkSize; k++){
                    ...
                    scoresM[relaxChunkSize*i + k] = MathOps::add_max(scoreDiag, fooArray[k], MathOps::max(E, scoresF[relaxChunkSize*i + k]));
        ...
        void stepSingleTileFirstDiagonal(int subject_letter, int row){
            ...
            #else
            shuffleScores(ScoreType{}, OOBScore<ScoreType>::get());
            #endif
    ...
    template<
        ...
        int relaxChunkSize = 4
    >
    struct SemiglobalAlignmentAffineGapState_half2OrShort2{
        using ScoreType = ScoreType_;
        ...
        __device__
        SemiglobalAlignmentAffineGapState_half2OrShort2(const SubstitutionScoreProvider& sub, Group& g, UpdateMaxInLastColumnOp& update, const ScoringKernelParam<ScoreType>& scoring
        ) : 
        ...
        void relaxFirstDiagonal_impl(int currentLetter, int row, int tileNr){
            ...
            if constexpr(isFirstTile){
                E = MathOps::add_max(scoreLeft, gapopenscore, MathOps::add(E, gapextendscore));
                #ifdef PRINT_STATE_SEMIGLOBAL_AFFINE_HALF2
                    printf("tid %d, compute E = (%f %f)\n", group.thread_rank(), float(E.x), float(E.y));
                #endif
            }else{
                if(group.thread_rank() > 0){
                    E = MathOps::add_max(scoreLeft, gapopenscore, MathOps::add(E, gapextendscore));
            ...
            E = MathOps::add_max(scoresM[numItems-1], gapopenscore, MathOps::add(E, gapextendscore));
            #ifdef PRINT_STATE_SEMIGLOBAL_AFFINE_HALF2
                printf("tid %d, compute advanced E = (%f %f)\n", group.thread_rank(), float(E.x), float(E.y));
            #endif
            for(int k = 0; k < numItems; k++){
                scoresF[k] = MathOps::add_max(scoresM[k], gapopenscore, MathOps::add(scoresF[k], gapextendscore));
            }
